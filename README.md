it is simple document mangement tool

Features
========
use to track the document of each product in organized manner
can create muitple products, under each product mutilple documents can be created & can be download0
Admin can add different roles, change the roles.


Roles
=====
ADMIN => only admin,can manages the roles and create products
PM(Product Manager) => can create product and create document
Contributor or Enigneer => only access the product and the document
